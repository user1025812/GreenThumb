...
//         {/* change to the species of trees later on. currently status of plants */}
//         <span style={{
//             marginTop: "0.75rem",
//             background: STATUS_COLORS[tree.status] ?? "#ccc",
//             color: "white",
...
//             <div style={{ fontSize: "1.2rem", color: "#333", lineHeight: "1.9", textAlign: "left", width: "100%", display: "flex", flexDirection: "column", justifyContent: "center" }}>
//                 <p><strong>Username</strong>: {tree.username}</p>
//                 <p><strong>Tree Species</strong>: {tree.species ?? tree.name}</p>
//                 <p><strong>Tree ID</strong>: {tree.treeID ?? tree.treeId}</p>
//                 <p><strong>Status</strong>: {tree.status}</p>
//                 <p><strong>Assigned Farmer</strong>: {tree.assignedFarmer ?? tree.farmer}</p>
//                 <p><strong>Last Update</strong>: {tree.lastUpdate}</p>
//                 <p><strong>Current Location</strong>: {tree.currentLocation ?? tree.location}</p>
//                 <p><strong>Stage</strong>: {tree.stage}</p>
//                 <p><strong>Next Update Due</strong>: {tree.nextUpdateDue ?? tree.nextUpdate}</p>
//                 {tree.photoUpload && (
//                     <img
//                         src={tree.photoUpload}
//                         alt="Tree progress"
...
//     //get trees from navigation state if coming from track form, otherwise use sample data
//     const location = useLocation();
//     const trees = location.state?.trees ?? sampleTrees;
//     const userName = trees[0]?.username ?? trees[0]?.name ?? "there";
...
//                 <div style={{ display: "flex", gap: "2rem" }}>
//                     {visibleTrees.map(tree => (
//                         <TreeCard key={tree.id ?? tree._id} tree={tree} onClick={setSelectedTree} />
//                     ))}
//                 </div>
